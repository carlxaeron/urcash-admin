<template>
  <q-page class="bg-secondary">
    ...
    <vue-json-to-csv
    :json-data="data"
    :labels="{ product_name: { title: 'Product' }, quantities_sold: { title: 'Sold Items' }, total_sales_of_merchant: { title: 'Total Sales' }, total_transactions: { title: 'Total Transactions' } }"
    :csv-title="'Report by Product'"
    :separator="';'">
    <q-btn
          color="primary"
          icon-right="archive"
        ...
        <q-select v-model="target" style="width:250px;" debounce="400" dense :options="targetlist"  option-value="ean"
          option-label="name"
          emit-value
          map-options
...
export default {
  ...
  data () {
    return {
      ...
      target: null,
      columns: [
        { name: 'product_name', align: 'left', label: 'Product', field: 'product_name', sortable: true },
  ...
  methods: {
    getData () {
      if (this.target !== null && this.date_from !== null && this.date_to !== null) {
        this.$axios.post('sales/products',
          {
            ean: this.target,
            from: this.date_from,
            to: this.date_to
